...
const PidWorker = {
    ...
    simulateModel(data) {
		...
		for(var i = 0; i < data.params.length; i++) {
	      arma_params.push([[data.params[i], 0]]);
		}
		// instantiate arma matrices
		var t_uniform = Arma.CxMat.from_array(arma_time  );
		var u_uniform = Arma.CxMat.from_array(arma_input );
		var y_uniform = Arma.CxMat.from_array(arma_output);
		var params    = Arma.CxMat.from_array(arma_params);
    	// detrended simulation
		var y_detrend = new Arma.cx_mat();
		params.resize(10, 1);
		pid.detrend_sim(data.type, params, t_uniform, u_uniform, y_uniform, y_detrend);
        // create return value
        var y = y_detrend.real().to_array().map(arr => arr[0]);
        return {
        	y: y,
        }
    }, // simulateModel
    ...
    makeSimulation(data) {
		...
		for(var i = 0; i < sim_length_r; i++) {
			// ref value
			var r_value = i > Math.ceil(data.r_time*sim_length_r) ? data.cached_r_size[i] : 0.0;
